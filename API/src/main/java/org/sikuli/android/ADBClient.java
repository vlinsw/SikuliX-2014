...
public class ADBClient {
	...
	private static void init() {
		...
		if (jadb != null) {
			...
			try {
				...
				e.printStackTrace();
				System.err.println("not devices.");
			}
	...
	private static void init(String serial) {
		getConnection(true);
		if (jadb == null) {
			try {
				new AdbServerLauncher().launch();
		...
		if (serial == null || serial.equalsIgnoreCase("")) {
			device = null;
			Debug.error("ADBClient: serial can't null or empty.");
		}
		if (jadb != null) {
			List<JadbDevice> devices = null;
			try {
	...
	public static JadbDevice getDevice(String serial) {
		init(serial);
		return device;
	}
