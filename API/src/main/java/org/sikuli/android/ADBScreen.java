...
//---------------------------Inits
	...
	public ADBScreen getADBScreen() {
		return this.screen;
	}

	public void setADBScreen(ADBScreen screen) {
		this.screen = screen;
	}
	...
	public static ADBScreen start() {
		try {
			...
			if (screen.getADBScreen() == null) {
				screen.setADBScreen(screen);
			}
			return screen.getADBScreen();
		} catch (Exception e) {
	...
	/**
	 * 原功能不支持指定设备。 修改为支持指定设备，并发可能会有问题。
	 * 
	 * @author zhangyuehua
	 * @date 2019/10/23
	 */
	public static ADBScreen start(String serial) {
		try {
			ADBScreen screen = new ADBScreen(serial);
			if (screen.getADBScreen() == null) {
				screen.setADBScreen(screen);
			}
			return screen.getADBScreen();
	...
	public ADBScreen() {
		...
		device = ADBDevice.instance().init();
		if (device != null) {
	...
	public ADBScreen(String serial) {
		super();
		setOtherScreen(this);

		device = ADBDevice.instance().init(serial);
		if (device != null) {
			robot = device.getRobot(this);
			robot.setAutoDelay(10);
			bounds = device.getBounds();
